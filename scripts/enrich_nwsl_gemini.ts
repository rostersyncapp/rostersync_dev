#!/usr/bin/env ts-node
/**
 * NWSL Roster Enrichment - Gemini AI
 * 
 ...
 * 
 * Usage: npx tsx scripts/enrich_nwsl_gemini.ts [team-id] [limit]
 * Example: npx tsx scripts/enrich_nwsl_gemini.ts angel-city-fc 5
 * Example: npx tsx scripts/enrich_nwsl_gemini.ts all 1000
 */
...
async function enrichPlayer(player: any, teamConfig: any): Promise<number> {
    ...
    // Prompt for Gemini
    const prompt = `
    You are a sports data expert. Provide the following for NWSL player "${player.player_name}" (Team: ${teamConfig.name}):
    1. Simple Phonetic Pronunciation (e.g. "Alex Morgan" -> "AL-eks MOR-guhn")
    2. International Phonetic Alphabet (IPA) (e.g. "Alex Morgan" -> "/ˈæl.ɪks ˈmɔːr.ɡən/")
    3. Chinese Character Spelling (e.g. "Alex Morgan" -> "艾利克斯·摩根")
    4. Hardware Safe Name (the player name in ALL CAPS, e.g. "ALEX MORGAN")
...
async function runEnrichment(teamId?: string, limit: number = 5) {
    ...
    const { data: teams } = await supabase.from('nwsl_teams').select('*');
