#!/usr/bin/env ts-node
/**
 * MLS Roster Enrichment - Gemini AI
 * 
 ...
 * 
 * Usage: npx tsx scripts/enrich_mls_gemini.ts [team-id] [limit]
 * Example: npx tsx scripts/enrich_mls_gemini.ts atlanta-united-fc 5
 * Example: npx tsx scripts/enrich_mls_gemini.ts all 1000
 */
...
async function enrichPlayer(player: any, teamConfig: any): Promise<number> {
    ...
    // Prompt for Gemini
    const prompt = `
    You are a sports data expert. Provide the following for MLS player "${player.player_name}" (Team: ${teamConfig.name}):
    1. Simple Phonetic Pronunciation (e.g. "Lionel Messi" -> "LEE-uh-nel MESS-ee")
    2. International Phonetic Alphabet (IPA) (e.g. "Lionel Messi" -> "/li.oˈnɛl ˈmɛsi/")
    3. Chinese Character Spelling (e.g. "Lionel Messi" -> "里奥内尔·梅西")
    4. Hardware Safe Name (the player name in ALL CAPS, e.g. "LIONEL MESSI")
...
async function runEnrichment(teamId?: string, limit: number = 5) {
    ...
    const { data: teams } = await supabase.from('mls_teams').select('*');
