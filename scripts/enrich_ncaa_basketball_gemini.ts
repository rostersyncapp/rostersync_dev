#!/usr/bin/env ts-node
/**
 * NCAA Men's Basketball Roster Enrichment - Gemini AI
 * 
 ...
 * 
 * Usage: npx tsx scripts/enrich_ncaa_basketball_gemini.ts [team-id] [limit]
 * Example: npx tsx scripts/enrich_ncaa_basketball_gemini.ts duke-blue-devils 5
 * Example: npx tsx scripts/enrich_ncaa_basketball_gemini.ts all 1000
 */
...
async function enrichPlayer(player: any, teamConfig: any): Promise<number> {
    ...
    // Prompt for Gemini
    const prompt = `
    You are a sports data expert. Provide the following for NCAA Men's Basketball player "${player.player_name}" (Team: ${teamConfig.name}):
    1. Simple Phonetic Pronunciation (e.g. "Cooper Flagg" -> "KOO-per FLAG")
    2. International Phonetic Alphabet (IPA) (e.g. "Cooper Flagg" -> "/ˈkuːpər flæɡ/")
    3. Chinese Character Spelling (e.g. "Cooper Flagg" -> "库珀·弗拉格")
    4. Hardware Safe Name (the player name in ALL CAPS, e.g. "COOPER FLAGG")
...
async function runEnrichment(teamId?: string, limit: number = 5) {
    ...
    const { data: teams } = await supabase.from('ncaa_basketball_teams').select('*');
