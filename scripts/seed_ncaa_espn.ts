/**
 * NCAA Football ESPN Roster Seeder
 * Scrapes NCAA Football roster data from ESPN and saves to Supabase.
 * Usage: npx tsx scripts/seed_ncaa_espn.ts [startYear] [endYear] [teamFilter]
 */
...
const NCAA_TEAMS: NCAATeam[] = [
    ...
    // Big 12
    { id: '12', name: 'Arizona', slug: 'arizona-wildcats', conference: 'Big 12', primaryColor: '#003366', secondaryColor: '#CC0033' },
    { id: '9', name: 'Arizona State', slug: 'arizona-state-sun-devils', conference: 'Big 12', primaryColor: '#8C1D40', secondaryColor: '#FFC627' },
    { id: '239', name: 'Baylor', slug: 'baylor-bears', conference: 'Big 12', primaryColor: '#154733', secondaryColor: '#FFB81C' },
    ...
    { id: '248', name: 'Houston', slug: 'houston-cougars', conference: 'Big 12', primaryColor: '#C8102E', secondaryColor: '#F1F2F3' },
    ...
    { id: '2306', name: 'Kansas State', slug: 'kansas-state-wildcats', conference: 'Big 12', primaryColor: '#512888', secondaryColor: '#D1D1D1' },
    { id: '197', name: 'Oklahoma State', slug: 'oklahoma-state-cowboys', conference: 'Big 12', primaryColor: '#FF6600', secondaryColor: '#000000' },
    ...
    { id: '254', name: 'Utah', slug: 'utah-utes', conference: 'Big 12', primaryColor: '#CC0000', secondaryColor: '#000000' },
    ...
    // Pac-2
    { id: '204', name: 'Oregon State', slug: 'oregon-state-beavers', conference: 'Pac-2', primaryColor: '#D73F09', secondaryColor: '#000000' },
    { id: '265', name: 'Washington State', slug: 'washington-state-cougars', conference: 'Pac-2', primaryColor: '#981E32', secondaryColor: '#5E6062' }
];

async function seedTeams() {
    console.log('🏈 Seeding NCAA Football teams...');
    for (const team of NCAA_TEAMS) {
        const { error } = await supabase.from('ncaa_football_teams').upsert({
            id: team.id,
            name: team.name,
    ...
    console.log('✅ Seeded NCAA Football teams');
}

async function scrapeRoster(teamId: string, season: number) {
    const url = `https://site.api.espn.com/apis/common/v3/sports/football/college-football/teams/${teamId}/roster?season=${season}`;
    ...
    try {
        ...
        if (!athleteGroups || !Array.isArray(athleteGroups) || athleteGroups.length === 0) {
            // console.log(`    ⚠️ No athletes array found in API response for ${teamId} ${season}`);
        ...
        if (players.length > 0) {
            // console.log(`    🔍 Fetched ${players.length} players for ${teamId} ${season}`);
        }
...
async function main() {
    ...
    for (const team of NCAA_TEAMS) {
        ...
        for (let season = startYear; season <= endYear; season++) {
            ...
            if (players.length > 0) {
                const { error } = await supabase.from('ncaa_football_rosters').upsert(players, {
    ...
    console.log('\n✅ NCAA Football ESPN Seeding Complete!');
}
