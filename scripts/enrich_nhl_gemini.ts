...
async function enrichPlayer(player: Player) {
    ...
    Provide the following details for the NHL player:
...
async function runEnrichment(teamId?: string, all: boolean = false, year?: number) {
    console.log(`🚀 Starting NHL Gemini Enrichment...`);

    let query = supabase.from('nhl_rosters').select('*');
    ...
    // Only enrich if phonetic_name is null
    query = query.is('phonetic_name', null);

    const { data: players, error } = await query.limit(50); // Batch limit for safety
    ...
    for (const player of players) {
        ...
        // Sleep to respect AI rate limits
...
runEnrichment(teamFlag, allFlag, year).then(() => {
    console.log('🏁 NHL Enrichment Complete!');
});
