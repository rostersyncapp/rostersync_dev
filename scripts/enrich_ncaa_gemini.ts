#!/usr/bin/env ts-node
/**
 * NCAA Football Roster Enrichment - Gemini AI
 * 
 ...
 * 
 * Usage: npx tsx scripts/enrich_ncaa_gemini.ts [team-id] [limit]
 * Example: npx tsx scripts/enrich_ncaa_gemini.ts alabama-crimson-tide 5
 * Example: npx tsx scripts/enrich_ncaa_gemini.ts all 1000
 */
...
async function enrichPlayer(player: any, teamConfig: any): Promise<number> {
    ...
    // Prompt for Gemini
    const prompt = `
    You are a sports data expert. Provide the following for NCAA Football player "${player.player_name}" (Team: ${teamConfig.name}):
    1. Simple Phonetic Pronunciation (e.g. "Shedeur Sanders" -> "shuh-DEER SAN-derz")
    2. International Phonetic Alphabet (IPA) (e.g. "Shedeur Sanders" -> "/ʃəˈdɪər ˈsændərz/")
    3. Chinese Character Spelling (e.g. "Shedeur Sanders" -> "谢德尔·桑德斯")
    4. Hardware Safe Name (the player name in ALL CAPS, e.g. "SHEDEUR SANDERS")
...
async function runEnrichment(teamId?: string, limit: number = 5) {
    ...
    const { data: teams } = await supabase.from('ncaa_football_teams').select('*');
