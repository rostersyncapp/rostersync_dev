/**
 * NCAA Women's Basketball ESPN Roster Seeder
 * Scrapes NCAA Women's Basketball roster data from ESPN and saves to Supabase.
 * Usage: npx tsx scripts/seed_ncaa_womens_basketball_espn.ts [startYear] [endYear] [confOrTeamFilter]
 */
...
const NCAA_WBB_TEAMS: NCAATeam[] = [
    ...
    { id: '61', name: 'Georgia', slug: 'georgia-lady-bulldogs', conference: 'SEC', primaryColor: '#BA0C2F', secondaryColor: '#000000' },
    ...
    { id: '2633', name: 'Tennessee', slug: 'tennessee-lady-volunteers', conference: 'SEC', primaryColor: '#FF8200', secondaryColor: '#FFFFFF' },
    ...
    { id: '213', name: 'Penn State', slug: 'penn-state-lady-lions', conference: 'Big Ten', primaryColor: '#041E42', secondaryColor: '#FFFFFF' },
    ...
    { id: '197', name: 'Oklahoma State', slug: 'oklahoma-state-cowgirls', conference: 'Big 12', primaryColor: '#FF7300', secondaryColor: '#000000' },
    { id: '2628', name: 'TCU', slug: 'tcu-horned-frogs', conference: 'Big 12', primaryColor: '#4D1979', secondaryColor: '#A3A9AC' },
    { id: '2641', name: 'Texas Tech', slug: 'texas-tech-lady-raiders', conference: 'Big 12', primaryColor: '#CC0000', secondaryColor: '#000000' },
...
async function seedTeams() {
    console.log('🏀 Seeding NCAA Women\'s Basketball teams...');
    for (const team of NCAA_WBB_TEAMS) {
        const { error } = await supabase.from('ncaa_womens_basketball_teams').upsert({
            id: team.id,
            name: team.name,
...
async function scrapeRoster(teamId: string, season: number) {
    const url = `https://site.api.espn.com/apis/common/v3/sports/basketball/womens-college-basketball/teams/${teamId}/roster?season=${season}`;
...
async function main() {
    console.log('\n🏀 ESPN NCAA Women\'s Basketball Seeding');
    ...
    for (const team of NCAA_WBB_TEAMS) {
        ...
        for (let season = startYear; season <= endYear; season++) {
            ...
            if (players.length > 0) {
                const { error } = await supabase.from('ncaa_womens_basketball_rosters').upsert(players, {
    ...
    console.log('\n✅ NCAA Women\'s Basketball ESPN Seeding Complete!');
}
