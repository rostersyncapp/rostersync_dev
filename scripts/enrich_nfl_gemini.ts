#!/usr/bin/env ts-node
/**
 * NFL Roster Enrichment - Gemini AI
 * 
 ...
 * 
 * Usage: npx tsx scripts/enrich_nfl_gemini.ts [team-id] [limit]
 * Example: npx tsx scripts/enrich_nfl_gemini.ts baltimore-ravens 5
 * Example: npx tsx scripts/enrich_nfl_gemini.ts all 1000
 */
...
async function enrichPlayer(player: any, teamConfig: any): Promise<number> {
    ...
    // Prompt for Gemini
    const prompt = `
    You are a sports data expert. Provide the following for NFL player "${player.player_name}" (Team: ${teamConfig.name}):
    1. Simple Phonetic Pronunciation (e.g. "Lamar Jackson" -> "luh-MAHR JAK-suhn")
    2. International Phonetic Alphabet (IPA) (e.g. "Lamar Jackson" -> "/ləˈmɑːr ˈdʒæk.sən/")
    3. Chinese Character Spelling (e.g. "Lamar Jackson" -> "拉玛尔·杰克逊")
    4. Hardware Safe Name (the player name in ALL CAPS, e.g. "LAMAR JACKSON")
