...
- **Implicit**: We rely on standard NHL team names being present in `ESPN_TEAM_IDS` or `KNOWN_TEAM_LOGOS`.
- **Ambiguity**: NHL teams score **3 points** in priority resolving (e.g., "Kings" -> LA Kings (NHL) vs Sacramento Kings (NBA) - Priority Equal, resolved by name length or context).
