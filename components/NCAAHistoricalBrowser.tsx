...
interface NCAAHistoricalBrowserProps {
...
export default function NCAAHistoricalBrowser({ onSave, userTier = 'FREE' }: NCAAHistoricalBrowserProps) {
    ...
    const [roster, setRoster] = useState<NCAAPlayer[]>([]);
    ...
    async function loadTeams() {
        const { data, error } = await supabase
            .from('ncaa_football_teams')
            .select('*')
            .order('display_name');

        if (error) {
            console.error('Error loading teams:', error);
            setError('Failed to load NCAA teams');
            return;
        }
    ...
    async function loadSeasons(teamId: string) {
        setLoading(true);
        const { data, error } = await supabase
            .rpc('get_ncaa_football_team_seasons', { p_team_id: teamId });
    ...
    function convertToAthletes(): Athlete[] {
        return roster.map((player) => ({
            ...
            phoneticSimplified: '',
            phoneticIPA: '',
            headshot: `https://a.espncdn.com/i/headshots/ncaa/players/full/${player.player_id}.png`,
            id: `ncaa-${player.player_id}-${selectedSeason}`,
        }));
    }
    ...
    function handleSaveToLibrary() {
        ...
        const newRoster: Roster = {
            ...
            teamMetadata: {
                primaryColor: selectedTeam.primary_color || '#9E1B32',
    ...
    function handleExportCSV() {
        ...
        const csvHeaders = ['Jersey Number', 'Player Name', 'Position', 'Height', 'Weight', 'Class'];
        ...
        a.href = url;
        a.download = `${selectedTeam.slug}_${selectedSeason}_roster.csv`;
        a.click();
        ...
        <div className="max-w-6xl mx-auto p-6">
            <div className="mb-8">
                <div className="flex items-center gap-4 mb-2">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">NCAA Football Archive</h1>
                        <p className="text-gray-600 dark:text-gray-400">Access historical NCAA Football rosters (2020-2025)</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 p-8 mb-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full px-5 py-4 bg-gray-50 dark:bg-gray-800 border-none rounded-2xl focus:ring-2 focus:ring-red-500 font-bold dark:text-white"
                        >
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full px-5 py-4 bg-gray-50 dark:bg-gray-800 border-none rounded-2xl focus:ring-2 focus:ring-red-500 font-bold dark:text-white disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                ...
                {selectedTeam && selectedSeason && (
                    <div
                        ...
                        style={{
                            backgroundColor: (selectedTeam.primary_color || '#9E1B32') + '15',
                            borderLeft: `6px solid ${selectedTeam.primary_color || '#9E1B32'}`
                        }}
                    >
                ...
                <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden animate-in fade-in duration-700">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/30 px-8 py-6 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row gap-4">
                        <button
                            ...
                            className="flex items-center justify-center gap-3 px-8 py-4 bg-red-700 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg shadow-red-500/25 hover:scale-[1.02] active:scale-[0.98] transition-all"
                        >
                            <Save size={20} />
                            Save to Library
                        </button>
            ...
            {loading && (
                <div className="text-center py-20">
                    <div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-red-700 border-t-transparent shadow-sm"></div>
                    <p className="mt-4 text-gray-400 font-bold uppercase tracking-widest text-sm">Fetching NCAA Football Roster...</p>
                </div>
            )}
